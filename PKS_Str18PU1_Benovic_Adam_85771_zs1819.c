...
void senderPart() {
	...
	receiverAddress.sin_addr.s_addr = inet_addr(destinationIP);
	...
	while (1) {		
		...
		printf("Ak este chcete posielat, stlacte 'c' a Enter\n");		
		...
		if (dataType == KEEP_ALIVE) {
			...
			while ((n = recvfrom(receivingSocket, buf, fragmentSize + HEADER_SIZE, 0, (SOCKADDR *)&senderAddr, &senderAddrSize)) &&
				  (bytesToInt(buf, 12) == FILE_FLAG && (bytesToInt(buf, 0) < bytesToInt(buf, 8)))) {
					if(checkChecksum(bytesToInt(buf, 4), buf + HEADER_SIZE, n - HEADER_SIZE))
